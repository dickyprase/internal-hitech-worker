...
export default function MedicalPage() {
  return <MedicalContent type='mc' />;
}

function MedicalContent({ type }: { type: 'mc' | 'ri' }) {
  ...
  // User profile for plafon calculation
  const [gajiPokok, setGajiPokok] = useState(0);
  const [statusPernikahan, setStatusPernikahan] = useState<string>('single');
  ...
  const icon =
    type === 'mc' ? <IconHeartbeat className='h-4 w-4' /> : <IconFileText className='h-4 w-4' />;

  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const [balRes, txRes, meRes] = await Promise.all([
        fetch(`/api/medical/balance?type=${type}`),
        fetch(`/api/medical/transactions?type=${type}`),
        fetch('/api/user/me')
      ]);
  ...
  // Plafon multiplier based on marriage status
  const getMultiplier = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess('Klaim berhasil disimpan');
  ...
  return (
    <div className='w-full max-w-full overflow-x-hidden space-y-6'>
      <PageHeader
        title={title}
        subtitle={`Kelola klaim ${title.toLowerCase()}`}
        actions={
      ...
      <div className='grid gap-4 md:grid-cols-2'>
          ...
          <CardContent className='p-5'>
            <div className='flex items-start justify-between'>
              <div className='space-y-1'>
                <p className='text-sm text-muted-foreground'>Plafon {title}</p>
        ...
        {/* Card Sisa Plafon — Dynamic Color */}
        <Card
          className={
            persentase >= 90
      ...
      <Card>
        <CardHeader>
          <CardTitle>Ajukan Klaim</CardTitle>
          <CardDescription>
            Isi form berikut untuk mengajukan klaim {title.toLowerCase()}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className='space-y-4'>
            ...
            <div className='space-y-2'>
              ...
              <Textarea
                ...
                placeholder='Contoh: Periksa gigi di klinik ABC'
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent className='sm:max-w-md'>
          <DialogHeader>
            ...
            <DialogDescription>Perbarui data klaim medical</DialogDescription>
          </DialogHeader>
